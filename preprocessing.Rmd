  ...
  mainPanel(
    ...
    tabsetPanel(
      tabPanel("Cleaned Dataset", fluidRow(
        column(12, tableOutput("cds")))),
      tabPanel("Reduced Dataset", fluidRow(
        column(12, tableOutput("rds")))),
      tabPanel("Full Dataset", fluidRow(
        column(12, tableOutput("fds"))))
    )
  )
)

output$cds <- renderTable(h.df)
output$rds <- renderTable(h.rdf)
output$fds <- renderTable(h.full)
```
